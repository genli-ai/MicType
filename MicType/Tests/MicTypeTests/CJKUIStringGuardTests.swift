...
/// 3. 下面两张白名单——按「文件:声明名」豁免整段非界面文本层，以及按「文件:字面量内容」
///    豁免唯一一处故意双语的界面文案。
///
...
final class CJKUIStringGuardTests: XCTestCase {
    ...
    private static let textLayerDeclarations: Set<String> = [
        ...
        "RecognitionLanguages.swift:hotwordSeparator",
        // 内置模型目录：中英文是 LocalizedText(zh:en:) 的两侧（显示时走 .localized → tr），
        // 不是漏了 tr() 的界面字面量。与仓库根 model-catalog.json 同源，得逐字对得上
        "ModelCatalog.swift:builtIn",
    ...
    /// 按「文件名:字面量内容」豁免的**故意例外**。加一条就写一行为什么。
    ...
    func testNoCJKLeaksInUIStrings() throws {
        ...
        for file in files {
            ...
            for hit in CJKSourceScanner.scan(source) {
                if Self.literalWhitelist.contains("\(file):\(hit.literal)") { continue }
        ...
        XCTAssertTrue(failures.isEmpty, """
            ...
            就加进 textLayerDeclarations（写清为什么）；故意双语才进 literalWhitelist。
            """)
    }
