...
final class MetricsTests: XCTestCase {
    ...
    /// 没润色的那些轮（档位关着 / 指令）不能当成 0 毫秒拉低中位数
    func testDigestSkipsSessionsWithoutPolish() {
        ...
        XCTAssertEqual(Metrics.digest(items, count: 10)?.polishMs, 2000)
    }
    ...
    func testSummaryLineDropsPolishSegmentWhenAbsent() {
        let withPolish = Metrics.summaryLine(
            Metrics.Digest(sampleCount: 10, asrMs: 320, polishMs: 1800, insertMs: 90))
        ...
        let withoutPolish = Metrics.summaryLine(
            Metrics.Digest(sampleCount: 3, asrMs: 320, polishMs: nil, insertMs: 90))
    ...
    /// 没润色的那一轮写 "-" 而不是 0——"没发生"和"零毫秒"必须一眼能分开。
    func testDiagnosticRowMarksMissingPolishWithDash() {
        ...
        XCTAssertTrue(row.contains("polish=-"))
