...
enum PolishService {
    ...
    static func systemPrompt(for level: PolishLevel) -> String {
        ...
        7. 数字写法——只换写法，数值一位都不许变：金额、数量、度量、百分比、日期、年份、时间、编号、电话、版本号、小数，用阿拉伯数字，数字与前后汉字之间不加空格（「一百零一块」→「101块」、「二零一一年三月五号」→「2011年3月5日」、「百分之二十」→「20%」、「三点五公里」→「3.5公里」、「一万二千」→「1.2万」）。成语与固定说法、概数、序数、不表精确数量的「一」保持汉字（一下、一些、一起、万一、十分重要、千万别、两三个、十几个、七八成、第一次、星期三、三心二意）；时间里的「半」「一刻」保持汉字（三点半→「3点半」）。英文里的数字保持识别结果原样的写法。
