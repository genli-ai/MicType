...
enum SettingsWindowSizing {
    ...
    /// **设置正页自己的地板**（5.0.3，用户 2026-09-23 拍板）。
    ///
    /// 两件事一起解决：
    ///   • 换服务商时窗口不许跳——阿里云比 OpenAI 多一行「API Host」（实测 306 vs 261），
    ///     点一下选择器窗口就矮 45 点再长回来，而内容一个字都没变；
    ///   • OpenAI 那一档不许在底栏下面留一块空白——底栏 5.0.3 起钉在窗底，
    ///     多出来的空当留在表和底栏之间。
    ///
    /// 320 是怎么来的：较高的那一家（阿里云 + 两条权限横幅）实测 306，留一点余量。
    /// 内容真比它高时照常长高——这是地板，不是天花板。
    static let overviewContentHeight: CGFloat = 320
